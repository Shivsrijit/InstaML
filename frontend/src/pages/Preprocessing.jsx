...
const Preprocessing = ({ datasetStatus, refreshStatus }) => {
  ...
  const getCategoricalColumns = () => {
    if (!datasetStatus?.columns) return [];
    return datasetStatus.columns.filter(c => 
      datasetStatus.dtypes?.[c]?.includes('object') || datasetStatus.dtypes?.[c]?.includes('category') || datasetStatus.dtypes?.[c]?.includes('bool')
    );
  };
      ...
      <div className="card" style={{ minHeight: '300px', position: 'relative' }}>
        ...
        {activeTab === 'outliers' && (
          <div>
            ...
            <button onClick={handleRemoveOutliers} className="btn btn-primary">
              Filter Outlier Rows
            </button>
          </div>
        )}
